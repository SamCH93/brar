...
%% \section{Asymptotic randomization probability}

%% Assume the normality setting from Section~\ref{sec:normal}, and suppose there is
%% one treatment group ($K = 1$), and that a normal prior centered around zero $\mu
%% = 0$ with variance $\tau^2$ along with $\Pr(H_0) > 0$ for the spike at 0 are
%% specified. For an effect estimate $\hat{\theta}_n$ with standard error
%% $\sigma_n$ based on $n$ observations, the posterior probability of the null
%% hypothesis $H_0$ is then
%% \begin{align*}
%%   \Pr(H_0 \mid \hat{\theta}_n) =
%%   \left(
%%   1 + \frac{1 - \Pr(H_0)}{\Pr(H_0)} \, \times \, \exp\left[\frac{Z^2_n}{2(1 + \sigma^2_n/\tau^2)}\right] \, \bigg / \,
%%   \sqrt{1 + \frac{\tau^2}{\sigma^2_n}}
%%   \right)^{-1}
%% \end{align*}
%% with \textit{z}-statistic $Z_n = \hat{\theta}_n/\sigma_n$. If data are sampled
%% based on non-adaptive randomization, it is clear that as the sample size $n$
%% increases, under $H_0 \colon \theta = 0$, the \textit{z}-statistic is
%% asymptotically $Z_n \sim \mathrm{N}(0, 1)$ and the standard error goes to zero,
%% so that $\Pr(H_0 \mid \hat{\theta}_n) \uparrow 1$ in probability as $\sigma_n
%% \downarrow 0$. However, under RAR, the distribution of $Z_n$ is more complicated
%% as the standard error is now also a random variable
