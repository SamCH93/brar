...
We now explain the general idea of point null Bayesian RAR, without going into
...
improvements may be possible by considering the exact distribution of the data.
This will be detailed for binary outcomes in Section~\ref{sec:binary}.
...
illustrates how the latter function can be used.
...
We now describe the design and results of our simulation study following the
