...
%% Cai2013 also use hypothesis testing though they do not use point null but only H+ and H-, also they not realize connection to Thompson sampling
...
decreases to zero (i.e., $\pi \to 1 - \Phi\{(0 - \mu_{*})/\tau_{*}\} = \Phi(\mu_*/\tau_*)$ as $\Pr(H_0) \searrow 0$).
...
of these individuals does not imply their endorsement of the paper.
